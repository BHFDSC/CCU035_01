...
log using "${log_files}ccu035_01_no_hist_cvd_final_analysis_cohort_2_ethinicity.doc", text replace
...
save "ccu035_01_no_hist_cvd_final_analysis_cohort_2.dta", replace

use "ccu035_01_no_hist_cvd_final_analysis_cohort_2.dta", clear
...
file open covid_di using "cox_cluster_estimates_ccu035_no_hist_cvd_ethinicity.csv", write replace 
...
ren ethnic_cat5 white
	...
	capture noisily stcox pred1 age_at_cohort_start female, cl(practice)
	if _rc != 0 continue
	...
	capture noisily stcox pred1 age_at_cohort_start female current_smoker ex_smoker obese_bmi surgery_lastyr_hes hypertension diabetes cancer liver_disease dementia ckd depression hist_cvd cvd_meds, cl(practice) // add covariates  
	if _rc != 0 continue
	...
	if _rc != 0 continue
	gen seventeen=exp(_b[pred1])
	gen eighteen=exp(_b[pred1] - abs(invnormal(0.025))*_se[pred1])
	gen nineteen=exp(_b[pred1] + abs(invnormal(0.025))*_se[pred1])
	gen twenty=2*normal(-abs(_b[pred1]/_se[pred1]))
	file open desc using "cox_cluster_estimates_ccu035_no_hist_cvd_ethinicity.csv",  write append
	...
	file close desc
	capture drop one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen twenty pred1 
	}
