...
# MAGIC 
# MAGIC --CREATE TABLE .ccu035_01_cvdoutcomes_infections AS
# MAGIC create or replace global temp view ccu035_01_cvdoutcomes_infections_all as 
# MAGIC 
# MAGIC with cte_alloutcomes as (
# MAGIC SELECT NHS_NUMBER_DEID, DATE as record_date, name, term, "gdppr" AS SOURCE, terminology, code
# MAGIC FROM .ccu035_01_infection_cvd_outcomes_gdppr_all
# MAGIC UNION
# MAGIC SELECT NHS_NUMBER_DEID , EPISTART as record_date, name, term, 'hes_apc' as SOURCE, terminology, code
# MAGIC --FROM .CCU035_01_infection_hesapcoutcomes
# MAGIC FROM .ccu035_01_infection_hesapcoutcomes_first_diagnosis_all
# MAGIC UNION
...
# MAGIC --FROM .ccu035_01_infection_death_outcomes
# MAGIC FROM .ccu035_01_infection_death_outcomes_first_position_all
...
# MAGIC FROM global_temp.ccu035_01_cvdoutcomes_infections_all

# COMMAND ----------

#%sql
#select * from global_temp.ccu035_01_cvdoutcomes_infections_all
...
# MAGIC 
# MAGIC CREATE or replace global temp view  ccu035_01_cvdoutcomes_infections_first_all AS
...
# MAGIC --FROM .ccu035_01_cvdoutcomes_infections 
# MAGIC FROM global_temp.ccu035_01_cvdoutcomes_infections_all
# MAGIC )
# MAGIC 
# MAGIC SELECT *
...
# MAGIC --FROM .ccu035_01_cvdoutcomes_first
# MAGIC from global_temp.ccu035_01_cvdoutcomes_infections_first_all
# MAGIC group by name
...
# MAGIC %sql
# MAGIC select * from global_temp.ccu035_01_cvdoutcomes_infections_first_all

# COMMAND ----------

index_dates=spark.table (f'.ccu035_01_start_and_end_date')

final=spark.table(f'global_temp.ccu035_01_cvdoutcomes_infections_first_all')\
  .join(index_dates, on='NHS_NUMBER_DEID', how='inner')
final.createOrReplaceGlobalTempView(f'ccu035_01_cvdoutcomes_infections_first_all')

# COMMAND ----------

# MAGIC %sql
# MAGIC create table .ccu035_01_cvdoutcomes_first_all as
# MAGIC select *
# MAGIC from global_temp.ccu035_01_cvdoutcomes_infections_first_all
...
# MAGIC 
# MAGIC --CREATE TABLE .ccu035_01_cvdoutcome_infections_summary_first AS
...
# MAGIC FROM .ccu035_01_cvdoutcome_infections_summary_first

# COMMAND ----------

# MAGIC %sql
# MAGIC /*SELECT * FROM .ccu035_01_cvdoutcome_infections_summary_first
...
covariates = spark.table(f'.ccu035_01_included_patients_allcovariates')\
...
end_date= spark.table(f'.ccu035_01_start_and_end_date')
...
df=spark.table(f'.ccu035_01_gdppr_')

# COMMAND ----------

# MAGIC %sql 
# MAGIC select * from .ccu035_01_gdppr_
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS .ccu035_01_final_analysis_cohort

# COMMAND ----------

# MAGIC %sql
# MAGIC create table .ccu035_01_final_analysis_cohort as
# MAGIC select *
...
# MAGIC 
# MAGIC SELECT count(Distinct NHS_NUMBER_DEID) as count
# MAGIC FROM .ccu035_01_final_analysis_cohort
